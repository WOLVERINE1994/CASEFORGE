...
const envMarketingMotionEnabled =
  (process.env.NEXT_PUBLIC_PREMIUM_MOTION_ENABLED === "true" ||
    process.env.NEXT_PUBLIC_PREMIUM_MOTION_ENABLED === "1") &&
  (process.env.NEXT_PUBLIC_MARKETING_MOTION_ENABLED === "true" ||
    process.env.NEXT_PUBLIC_MARKETING_MOTION_ENABLED === "1");

const staticState: MarketingMotionState = {
...
function readMarketingMotionState(): MarketingMotionState {
  ...
  return {
    documentVisible,
    enabled: envMarketingMotionEnabled && !prefersReducedMotion && documentVisible,
    hasFinePointer,
    isTouchLike,
...
export default function useMarketingMotion(): MarketingMotionState {
  const [state, setState] = useState<MarketingMotionState>(staticState);

  useEffect(() => {
    if (!envMarketingMotionEnabled) return;
    ...
    const updateState = () => {
      setState(readMarketingMotionState());
    };
